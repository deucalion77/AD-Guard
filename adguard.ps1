param(
  ...
  # IMPORTANT: default is set at runtime based on script location (see below)
  [string]$OutDir,
...
# Tool root = folder where adguard.ps1 lives
$root = Split-Path -Parent $MyInvocation.MyCommand.Path
$scripts = Join-Path $root "scripts"
$engine  = Join-Path $root "engine"


# If OutDir not provided: use "<tool_root>\out"
if (-not $PSBoundParameters.ContainsKey('OutDir') -or [string]::IsNullOrWhiteSpace($OutDir)) {
  $OutDir = Join-Path $root "out"
}
# If OutDir is relative: resolve it against "<tool_root>"
...
switch ($Command) {
  ...
  "analyze" {
    ...
    $pyArgs = @(
      (Join-Path $engine "analyze.py"),
      "--raw", $rawDir,
      "--out", $scanJson,
      "--html", $htmlOut
    )

    if ($RulesDir -and $RulesDir.Trim()) {
      $pyArgs += @("--rules-dir", $RulesDir)
    }
    if ($CustomOnly) {
      $pyArgs += "--custom-only"
    }
